...
namespace std
{
   ...
   dialog_about::dialog_about(QWidget* owner, unicodestring const& caption) :
      dialog{ owner, Qt::WindowFlags{}, caption }
   {
   }
   //-----------------------------------------------------------------------------------------------//
   bool dialog_about::create(dialog* dlg)
   {
      ...
      //
      queue->get_layout()->addWidget(new label{ this, app_name + " " + version }, 0, Qt::AlignCenter);
      queue->get_layout()->addWidget(new label{ this, name_copy }, 0, Qt::AlignCenter);
      queue->get_layout()->addWidget(new label{ this, domain }, 0, Qt::AlignCenter);
      queue->get_layout()->addStretch();
      queue->get_layout()->addStretch();
      queue->get_layout()->addWidget(m_ok, 0, Qt::AlignCenter);
