...
using namespace std;
//-----------------------------------------------------------------------------------------------//
menu::menu(menu::widget_parent* parent, unicodestring const& caption, unicodestring const& name) :
   QMenu{ caption, owner }
{
...
menu* menu::create(menuitems* items, action* sender)
{
   for_each(items->begin(), items->end(), [=](menuitemdata* d)->auto
   {
      ...
      else
      {
         ...
         if (!d->is_submenu())
         {
            addMenu(create_sub(d->get_submenuitemdata(), sender));
         }
         else
         {
...
//-----------------------------------------------------------------------------------------------//
menu* menu::create_sub(list_values* sub_items, action* sender)
{
   ...
   for_each(sub_items->begin(), sub_items->end(), [=](menuitemdata* d)
   {
      ...
      else
      {
         ...
         if (d->is_submenu())
         {
            p->addMenu(create_sub(d->get_submenuitemdata(), cmd));
         }
         else
         {
            cmd->on_command = [=](bool const& checked, implement::action* sender)
            {
